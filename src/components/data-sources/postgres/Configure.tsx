...
const Configure: React.FC = () => {
  ...
  } = useForm<PostgresConfigForm>({
    resolver: zodResolver(postgresConfigSchema),
    defaultValues: {
      connectionUrl: "",
      tableName: "",
      host: "",
  ...
  const onSubmit = async (data: PostgresConfigForm) => {
    try {
      console.log("Form data:", data);
      // Handle save logic here
      // await savePostgresConfiguration(data);
    } catch (error) {
  ...
  return (
    <div className="w-full h-auto bg-white p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        {/* Table Name and Host Row */}
        <div className="grid grid-cols-2 gap-4">
          {renderInput("tableName", "Table Name", "eg, data_table")}
          ...
          {showSchemaExample && (
            <div className="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200">
              ...
              <CodeHighlighter
                language="json"
                code={`{
  "connectionUrl": "postgres://user:pass@host:port?db",
