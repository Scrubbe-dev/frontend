...
type GcpConfigForm = z.infer<typeof gcpConfigSchema>;
...
const Configure: React.FC = () => {
  ...
  } = useForm<GcpConfigForm>({
    resolver: zodResolver(gcpConfigSchema),
    defaultValues: {
      serviceAccountKey: "",
      projectId: "",
      host: "",
  ...
  const onSubmit = async (data: GcpConfigForm) => {
    try {
      console.log("Form data:", data);
      // Handle save logic here
      // await saveGcpConfiguration(data);
    } catch (error) {
  ...
  return (
    <div className="w-full h-auto bg-white p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {/* Service Account Key and Project ID Row */}
        <div className="grid grid-cols-2 gap-4">
          ...
          {renderInput("projectId", "Project ID", "GCP project ID")}
        </div>

        {/* Host (Full Width) */}
        <div>{renderInput("host", "Host", "eg, db.example.com")}</div>

        {/* Port and Username Row */}
        <div className="grid grid-cols-2 gap-4">
          {renderInput("port", "Port", "eg, 5432")}
          {renderInput("username", "Username", "eg, Scrubbe_user")}
        </div>
          ...
          {showSchemaExample && (
            <div className="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200">
              ...
              <CodeHighlighter
                language="json"
                code={`{
  "serviceAccountKey": "{\\"type\\": \\"service_account\\", \\"project_id\\": \\"your-project\\", \\"private_key_id\\": \\"****\\", \\"private_key\\": \\"****\\"}",
