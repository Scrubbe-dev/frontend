...
const Configure: React.FC = () => {
  ...
  const onSubmit = async (data: ApiConfigForm) => {
    try {
      ...
      // await saveApiConfiguration(data);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          {renderInput(
            "apiEndpoint",
            "API end point",
            "http://api.pexample.com"
          )}
        </div>

        {/* API Key and Host Row */}
        <div className="grid grid-cols-2 gap-4">
          {renderInput("apiKey", "API Key", "Api key")}
          ...
          {showSchemaExample && (
            <div className="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200">
              ...
              <CodeHighlighter
                language="json"
                code={`{
  "apiEndpoint": "http://api.pexample.com",
